...
def main():
    ...
    print(
        '\nTrain data shape and max dens:', train_data_clean.shape, train_data_clean.max(),
        '\nTest data shape and max dens:', test_data_clean.shape, test_data_clean.max(),
        '\nDynamic data shape and max dens:', f_dynamic_clean.shape, f_dynamic_clean.max(),
        )
    ...
    train_dataset = denoiser_data_load.Dataset(train_data_clean, train_data_clean, 
                                               rot_p=rot_p, rot_angles=rot_angles)
    dataloader = denoiser_data_load.DataLoader(train_dataset, batch_size=8, 
                                               shuffle=True, num_workers=0)

    val_dataset = denoiser_data_load.Dataset(test_data_clean, test_data_clean, 
                                             rot_p=rot_p, rot_angles=rot_angles)
    ...
    t = trange(
        Nepoch, desc='Loss - Train: %.2e Reg: %.2e Val: %.2e'%(0,0,0), leave=True)
    ...
    for epoch in t:
        ...
        for i_batch, sample_batch in enumerate(dataloader):
            ...
            transformed = noise_transform(
                transformed, max_dens=sample_batch['noisy'].max(), 
                noise_std=noise_std_rand_train)
            ...
            if lambda_jr != 0:
                reg_loss_max = compute_reg(denoised, sample_batch['noisy'], model, 
                                        reg_fun, epsilon=epsilon)
        ...
        with torch.no_grad():
            ...
            for i_batch, sample_batch in enumerate(val_dataloader):
                ...
                transformed = limited_view_tx(sample_batch['noisy'], P_set)
                transformed = blur_tx(transformed, tx)
                transformed = noise_transform(transformed, max_dens=sample_batch['noisy'].max(), noise_std=noise_std_rand_val)
        ...
        if obj_type in ['material', 'polymer_binary', 'polymer', 'polymer_binary_normalized']:
            with torch.no_grad():
                ...
                for i_batch, sample_batch in enumerate(dynamic_dataloader):
                    ...
                    transformed = limited_view_tx(sample_batch['noisy'], P_set)
                    transformed = blur_tx(transformed, tx)
                    transformed = noise_transform(transformed, max_dens=sample_batch['noisy'].max(), noise_std=noise_std_rand_dyn)
        ...
        if epoch > Nepoch//8 and loss_val_epoch[-1] == min(loss_val_epoch):
            torch.save(model, path+'%s_model_%s_%s_epochs_%d_num_layers_%d_num_ch_%d_noise_std_%.1e.pt' %(
                denoiser_type, obj_type, noise_est_type, Nepoch, numLayers, numChannels, std_high))

    save_results(loss_train_epoch, loss_val_epoch, loss_dyn_epoch,
                 train_data_clean, args)


def save_results(loss_train_epoch, loss_val_epoch, loss_dyn_epoch,
                 train_data_clean, args):
    Path(args.save_fig_path).mkdir(parents=True, exist_ok=True)
    filename = (f'{args.obj_type}_{args.denoiser_type}_noise_{args.std_low}_{args.std_high}_est_type_{args.noise_est_type}'
...
def generate_mask(args):
    ...
    for i in range(args.spatial_dim):
        for j in range(args.spatial_dim):
            if (i-args.spatial_dim//2)**2 + (j-args.spatial_dim//2)**2 >= (args.spatial_dim//2)**2:
                mask[0,0,i,j] = 0
    return mask
    ...
    for i in range(f.shape[0]):
        ...
        f[i] = alpha * torch.Tensor(
            iradon(g, theta=360*theta/(2*np.pi), filter_name='ramp')).view(
                1, f.shape[2], f.shape[3]).cuda() + (1 - alpha) * f[i]
    return f
...
def load_data(obj_type, std_high, std_low, noise_std, path):
    if 'walnut' in obj_type:
        ...
        train_path_2 = '0mm'
        test_path = '0mm'
        train_path = train_path_1 + '_' + train_path_2 if not train_path_2 == '' else train_path_1

        train_data_clean = np.load(path+'material_mean_corr_%s_train_gt.npy' %train_path)
        test_data_clean = np.load(path+'material_mean_corr_%s_test_gt.npy' %test_path)
        f_dynamic_clean = np.load(path+'material_mean_corr_dynamic_gt.npy')
        
        # train_data_noisy_rand = np.load(path+'material_mean_corr_%s_train_noise_std_rand_low_%.2e_high_%.2e.npy' %(train_path, std_low, std_high))
    ...
    return train_data_clean, test_data_clean, f_dynamic_clean, # train_data_noisy_rand, test_data_noisy_rand, f_dynamic_noisy_rand
...
parser.add_argument("--denoiser_type", type=str, required=True)  # ['dncnn', 'dncnn_oracle', 'dncnn_deblur']
...
parser.add_argument("--path", type=str, required=False, default='/home/berk/Desktop/spatio_temporal/2D_time_variant_tomography/obj_domain_psm/data/denoiser/')
parser.add_argument("--save_fig_path", type=str, required=False, default='/home/berk/Desktop/spatio_temporal/2D_time_variant_tomography/2D_neural_fields/own_imp/paper_figures/denoiser/')
