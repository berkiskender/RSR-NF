...
from skimage.transform import PiecewiseAffineTransform, iradon, radon, warp
...
def construct_pi_symm_g(g_radon, g_radon_pi_symm, ang_range):
    if ang_range == np.pi:
        # Concatenate pi-symm measurements as new columns
        g_radon_symm = np.zeros([g_radon.shape[0], 2 * g_radon.shape[1]])
        g_radon_symm[:, : g_radon.shape[1]] = g_radon
        g_radon_symm[:, g_radon.shape[1] :] = g_radon_pi_symm
        ...
        g_radon_symm, g_radon_symm_long = g_radon, g_radon
    return g_radon_symm, g_radon_symm_long
...
def generate_f_pol(
    ...
    for t in range(num_instances):
        f_pol_s[..., t] = radon(f[..., t * P // num_instances], theta=360 * theta_exp / (2 * np.pi))
        f_true_recon[..., t] = iradon(
            f_pol_s[..., t * P // num_instances], theta=360 * theta_exp / (2 * np.pi)
        )
    if save:
        str_period = '' if period is None else '_' + str(period)
        np.save(path + 'f_pol_s_%s%s_%d%s.npy' % (obj_type, add_path, P, str_period), f_pol_s)
        np.save(
            path + 'f_true_recon_%s%s_%d%s.npy' % (obj_type, add_path, P, str_period), f_true_recon
        )
...
def add_meas_noise(
    ...
    if save:
        if ang_range == np.pi:
            ...
            np.save(
                path
                + 'g_radon_symm_long_noisy_%s%s_%d%s_noise_std_%.2e.npy'
...
def denoising_network_loader(
    ...
    if train_type == 'dncnn':
        if denoiser_type == 'full_img':
            ...
            model_path = os.path.join(
                ...
                model_name
                + '_model_%s_%s_epochs_%d_num_layers_%d_num_ch_%d.pt'
                % (obj_type, noise_est_type, epochs, num_layers, num_channels),
            )
            model_dncnn = models_denoiser.dncnn(
                num_layers, num_channels, filterSize, noise_est_type
            ).cuda()
            patchifier_red = None
            ...
            model_path = os.path.join(
                'data/restoration_operator',
                model_name
                + '_model_%s_%s_num_layers_%d_patch_size_%d_patch_stride_%d_num_ch_%d_epochs_%d.pt'
                % (obj_type, noise_est_type, num_layers, pSize, pStride, num_channels, epochs),
            )
    ...
    print(
        'DnCNN %s Model path: {%s} Params number: %d'
...
def load_radon_op(pi_symm, spatial_dim, P, period=None, path=None):
    ...
    if pi_symm:
        R = np.load(
            path + 'A_radon_spatial_dim_%d_P_%d_bit_reversal_pi_symm.npy' % (spatial_dim, period)
        )
    else:
        R = np.load(path + 'A_radon_spatial_dim_%d_P_%d_bit_reversal.npy' % (spatial_dim, period))
...
def piecewise_affine_transform(image, mag_sin, spatial_dim):
    ...
    dst_rows = (
        src[:, 1] - np.sin(np.linspace(0, 3 * np.pi, src.shape[0])) * mag_sin * spatial_dim / 64
    )
    dst_cols = src[:, 0]
    dst_rows *= 1.5
