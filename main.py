...
def main():
    ...
    theta_exp = utils.generate_theta(num_meas, ang_range=ang_range, period=ang_period)[view_ang_sch]
    ...
    if gen_g_all:
        ...
        f_pol_radon = np.load(
            f_pol_path
            + 'f_pol_s_%s%s_%d%s%s.npy' % (obj_type, add_path, num_frames, str_period, str_rep)
        )
    ...
    for combo in itertools.product(
        ...
        params['num_channels_sweep'],
        params['pos_enc_type_sweep'],
        params['mapping_size_sweep'],
        params['scale_sweep'],
        params['lmbda_sweep'],
        params['beta_sweep'],
        params['reg_t_weight_sweep'],
        params['init'],
        params['t_freq_ratio_sweep'],
        params['act_type'],
        params['num_epoch_sweep'],
        params['num_primal_iter_sweep'],
        params['lr_primal_sweep'],
        params['reg_t_loss_type'],
        params['num_red_iter'],
        params['sch_thr_step_ratio'],
        params['sch_decay_ratio'],
        params['model_reg_weight'],
        params['type_model_reg'],
        params['sgd_size'],
        params['noise_std'],
        params['rep'],
        params['reg_mode'],
        params['num_layers_denoiser'],
        params['num_channels_denoiser'],
        params['lambda_jr'],
        params['final_act'],
        params['denoiser_model'],
    ):
        hp = HParams(*combo)
        print(hp)
        ...
        xyt_grid = torch.tensor(
            utils._create_yxt_grid((spatial_dim, spatial_dim, num_frames)).transpose(3, 0, 1, 2)[
                None, ...
            ],
            dtype=torch.float32,
            device=device,
        ...
        model_denoiser, patchifier = utils.denoising_network_loader(
            ...
            num_layers=hp.num_layers_denoiser,
            spatial_dim=spatial_dim,
            obj_type=denoiser_obj_type,
            num_channels=hp.num_channels_denoiser,
        ...
        progress = tqdm(
            range(hp.num_epoch),
            desc=(
                'PSNR:%.3e/%.3e SSIM:%.2e/%.2e MAE:%.2e/%.2e HFEN:%.2e/%.2e grad_nf:%.3e'
        ...
        for epoch in progress:
            [loss_epoch, f_nf_est, g_f_est] = train.learn_NF_SGD_multiview(
                ...
                reg_t_loss_type=hp.reg_t_loss_type,
                ...
                if hp.reg_mode == 'RED_reg' and (hp.beta != 0 and hp.lmbda != 0):
                    for _ in range(hp.num_red_iter):
                        model.f_est, red_metrics = train.f_update(
                            ...
                            model_denoiser,
                            model.f_est.squeeze(),
                            f_nf_est.detach().squeeze(),
                            f_cuda[..., :: hp.rep],
                            model.gamma_est.squeeze(),
                ...
                if epoch % params['metric_upd_freq'] == 0:
                    if plot:
                        plots.train_visualization(
                            loss_epoch,
                            model.f_est.detach().cpu(),
                            f_nf_est.squeeze().detach().cpu(),
                            model.gamma_est.detach().cpu(),
                            f,
                            num_frames // 8,
                    ...
                    progress.set_description(
                        ...
                        % (
                            metrics_dict['PSNR_f'][-1],
                            metrics_dict['PSNR_nf'][-1],
                            metrics_dict['SSIM_f'][-1],
                            metrics_dict['SSIM_nf'][-1],
                            metrics_dict['MAE_f'][-1],
                            metrics_dict['MAE_nf'][-1],
                            metrics_dict['HFEN_f'][-1],
                            metrics_dict['HFEN_nf'][-1],
    ...
    plots.plot_and_save_red_nf_results(
        ...
        num_frames,
        update_type,
        noise_std=noise_std_cli,
        ang_period=ang_period,
        pos_enc_type=hp.pos_enc_type,
        mapping_size=hp.mapping_size,
        scale=hp.scale,
        save_fig=True,
