...
class NeuralFieldModel(nn.Module):
    def __init__(self, input_ch=2, output_dim=1, num_layers=4, num_channels=256,
                 act_type='sine'):
        super(NeuralFieldModel, self).__init__()
        ...
        layers = [nn.Conv2d(input_ch, num_channels, kernel_size=1, bias=True),
                  self.act()]
        for _ in range(num_layers-1):
            layers += [nn.Conv2d(num_channels, num_channels,
                                 kernel_size=1, bias=True), 
                       self.act(),
                       nn.BatchNorm2d(num_channels)]
        layers += [nn.Conv2d(num_channels, output_dim, kernel_size=1),
...
class NeuralFieldModel3D(nn.Module):
    def __init__(self, input_ch=3, output_dim=1, num_layers=4, num_channels=256,
                 spatial_dim=128, mask=True, P=32, act_type='sine'):
        super(NeuralFieldModel3D, self).__init__()
        self.output_dim = output_dim
        self.num_layers = num_layers
        self.num_channels = num_channels
        ...
        layers = [nn.Conv3d(input_ch, num_channels, kernel_size=1, bias=True),
                  self.act]
        for _ in range(num_layers-1):
            layers += [nn.Conv3d(num_channels, num_channels,
                                 kernel_size=1, bias=True), 
                       self.act,
                       nn.BatchNorm3d(num_channels)]
        layers += [nn.Conv3d(num_channels, output_dim, kernel_size=1),
                   nn.Sigmoid(),
                  ]
        self.net = nn.Sequential(*layers)
...
class NeuralFieldModel3D_fc(nn.Module):
    def __init__(self, input_ch=3, output_dim=1, num_layers=7, num_channels=64, 
                 spatial_dim=128, mask=True, P=32, act_type='relu'):
        super(NeuralFieldModel3D_fc, self).__init__()
        self.output_dim = output_dim
        self.num_layers = num_layers
        self.num_channels = num_channels
        ...
        layers = [nn.Linear(input_ch, num_channels, bias=True), 
                  self.act]
        for _ in range(num_layers-1):
            layers += [nn.Linear(num_channels, num_channels, bias=True), 
                       self.act, 
                       nn.BatchNorm1d(num_channels)]
        layers += [nn.Linear(num_channels, output_dim), 
                   # nn.Sigmoid(),
                   nn.ReLU(),
                  ]
        self.net = nn.Sequential(*layers)
        ...
        # FOV mask
        self.mask = torch.autograd.Variable(
            torch.ones(spatial_dim, spatial_dim).cuda(), requires_grad=False).cuda()
        if mask:
            for i,j in list(itertools.product(
                np.arange(spatial_dim), np.arange(spatial_dim))):
                if (i-spatial_dim//2)**2 + (
...
class NeuralFieldModel3D_fc_sinogram(nn.Module):
    def __init__(self, input_ch=3, output_dim=1, num_layers=4, num_channels=256,
                 spatial_dim=128, mask=True, P=32, act_type='sine'):
        super(NeuralFieldModel3D_fc_sinogram, self).__init__()
        self.output_dim = output_dim
        self.num_layers = num_layers
        self.num_channels = num_channels
        ...
        layers = [nn.Linear(input_ch, num_channels, bias=True), 
                  self.act, nn.BatchNorm1d(num_channels)]
        for _ in range(num_layers-1):
            layers += [nn.Linear(num_channels, num_channels, bias=True), 
                       self.act, 
                       nn.BatchNorm1d(num_channels)]
...
class SineEncoding(nn.Module):
    ...
    def forward(self, coords):
        return torch.cat((torch.sin(2*torch.pi*coords), torch.cos(2*torch.pi*coords)), dim=1)

    
class GaussianFourierEncoding(nn.Module):
    def __init__(self, input_dim, mapping_size, scale):
        super(GaussianFourierEncoding, self).__init__()
        self.input_dim = input_dim
...
class GaussianFourierEncoding3D(nn.Module):
    def __init__(self, input_dim, mapping_size, scale):
        super(GaussianFourierEncoding3D, self).__init__()
        self.input_dim = input_dim
        self._mapping_size = mapping_size
        self._B = (torch.randn(input_dim, mapping_size) * scale).cuda()
    ...
    def forward(self, coords):
        ...
        coords = coords @ self._B
        coords = coords.reshape(batches, height, width, P, self._mapping_size).permute(0,4,1,2,3)
        return torch.cat((torch.sin(2*torch.pi*coords), torch.cos(2*torch.pi*coords)), dim=1)
...
class FourierEncoding3D(nn.Module):
    def __init__(self, input_dim, mapping_size):
        ...
        self.input_dim = input_dim
        self._mapping_size = mapping_size
        self.coefs = (torch.arange(start=1, end=mapping_size+1e-12) * torch.pi * 0.5).view(1, mapping_size, 1, 1, 1).cuda()
...
class FourierEncoding2D_t(nn.Module):
    def __init__(self, input_dim, mapping_size, t_freq_ratio):
        super(FourierEncoding2D_t, self).__init__()
        
        self.input_dim = input_dim
        self._mapping_size = mapping_size
        self.coefs_spatial = (
            torch.arange(start=1, end=mapping_size+1e-12) * torch.pi * 0.5).view(
            1, mapping_size, 1, 1, 1).cuda()
        self.coefs_temporal = (
            torch.arange(start=1, end=mapping_size+1e-12) * torch.pi * 0.5 * t_freq_ratio).view(
            1, mapping_size, 1, 1, 1).cuda()
        
    def forward(self, coords):
        arg_spatial = torch.kron(
            self.coefs_spatial, coords[:, :2, :, :, :].contiguous())
        arg_temporal = torch.kron(
            self.coefs_temporal, coords[:, 2:, :, :, :].contiguous())
        return torch.hstack((
...
class RedStaticRecon(nn.Module):
    def __init__(self, spatial_dim, mask=False):
        ...
        """
        super(RedStaticRecon, self).__init__()
...
class RedStaticReconFull(nn.Module):
    def __init__(self, spatial_dim, P, f_init=None, mask=False):
        ...
        mask (bool): If True, apply FOV mask for tomographic objects.
        """
        super(RedStaticReconFull, self).__init__()
...
class NF3DProjDomain(nn.Module):
    def __init__(self, input_ch=3, output_dim=1, num_layers=4, num_channels=256,
                 spatial_dim=128, P=32, act_type='sine', P_static=128, init=None,
                 w0=10, s0=5):
        super(NF3DProjDomain, self).__init__()
        self.output_dim = output_dim
        self.num_layers = num_layers
        self.num_channels = num_channels
        ...
        layers = [nn.Linear(input_ch, num_channels, bias=True), self.act, nn.BatchNorm1d(num_channels)]
        for _ in range(num_layers-1):
            layers += [nn.Linear(num_channels, num_channels, bias=True), 
                       self.act, 
                       nn.BatchNorm1d(num_channels)
...
class NeuralFieldModel3D_fc(nn.Module):
    def __init__(self, input_ch, output_dim, num_layers, num_channels, 
        ...
        layers += [nn.Linear(num_channels, output_dim, bias=True),
                   nn.ReLU(),
                  ]
        self.net = nn.Sequential(*layers)
...
class NeuralFieldModel3D_fc_res(nn.Module):
    ...
    def forward(self, coords):
        ...
        x = torch.unflatten(x, -2, (coords.shape[2], coords.shape[3], coords.shape[4])).permute(3, 0, 1, 2)[None, ...]
        return self.mask[None, None, ..., None] * x