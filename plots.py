...
def red_visualization(metrics: dict, plot: bool) -> None:
    ...
    for label, linestyle in zip(['red_in', 'denoised', 'red_out', 'dual'], ['--', '-.', '-', '--']):
        plt.plot(
            np.log(metrics[f'f_{label}_norm_err']), label=label, linestyle=linestyle, linewidth=0.5
        )
    plt.grid()
    plt.legend()
    plt.show()
...
def plot_and_save_red_nf_results(
    ...
    for cnt, res in enumerate(results_sorted):
        ...
        plt.plot(
            np.log10(np.array(res[1]['grad_nf']) + 1e-10), label=label_str, linestyle=linestyle
        )
        plt.grid(linestyle='--', linewidth=0.5)
        plt.legend(bbox_to_anchor=(0.75, -0.25), ncol=3)
    plt.ylim(-3, 1)
